// Per-lane RNA-seq transcriptome alignment stats row. Copy of tx_stats_individual.nf
// with an rnaseq_transcriptome_individual.csv output name. Identical parsing logic.
process RNASEQ_TX_STATS_INDIVIDUAL {
    ...
    output:
    tuple val(meta), path("${meta.id}.${meta.lane}.rnaseq_transcriptome_individual.csv"), emit: csv
...
with open('${prefix}.rnaseq_transcriptome_individual.csv', 'w') as fh:
    fh.write(',${prefix}\\n')
    for k, v in rows:
        fh.write(f'{k},{v}\\n')
PYEOF
    ...
    def prefix = "${meta.id}.${meta.lane}"
    """
    printf ',${prefix}\\ntotal_reads,0\\n' > ${prefix}.rnaseq_transcriptome_individual.csv
    """
}
