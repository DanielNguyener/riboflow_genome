...
docker run --rm danielnguyener/riboflow:latest apt list --installed 2>/dev/null > ./apt.list
docker run --rm danielnguyener/riboflow:latest bash -c "source activate ribo_genome && conda list" > ./conda.list
