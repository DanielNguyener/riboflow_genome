...
workflow TRANSCRIPTOME_ALIGN {
    ...
    ch_qpass_counts = SAMTOOLS_QPASS.out.counts  // [ meta(lane), total, primary, secondary ]
    ...
    if (dedup == 'position') {
        ...
        // Fan merged post-dedup BED back to lanes → per-lane BED + counts.
        ...
        // Split merged dedup BAM back to per-lane BAMs + counts.
    ...
    qpass_counts            = ch_qpass_counts                // [ meta(lane), total, primary, secondary ]
